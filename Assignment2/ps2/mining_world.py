...
class Map(object):
    ...
    def get_score(self, pos):
        score = 0
        # Is there a mineral?
        key = str(tuple(int(x) for x in tuple(pos)))
        if key in self.minerals:
            score += 25

        # Is there too much heat?
        index = tuple(int(x) for x in tuple(pos))
    ...
    def transition(self, pos, action):
        ...
        pos_tuple = tuple(int(x) for x in tuple(pos))
        pos_tuple_new = tuple(int(x) for x in tuple(new_pos))

        transition = (pos_tuple, pos_tuple_new)
    ...
    def update(self, new_points):
        ...
        if new_points:
            if np.random.random() < self.p_new_hot_spot:
                ...
                pos_ = tuple(int(x) for x in tuple(pos))
                key = str(pos_)
            ...
            for pos in indx:
                pos_tuple = tuple(int(x) for x in tuple(pos))
                if pos_tuple not in self.minerals:
                    pos_ = tuple(int(x) for x in pos)
                    self.minerals[str(tuple(pos_))] = self.generate_mineral(pos_)
